
/// Version Number
#define FCVersionMajor "0"
#define FCVersionMinor "10"
#define FCVersionName  "Dagoba"

#define FCRevision      "2862"      //Highest committed revision number
#define FCRevisionDate  "2010/01/16 16:35:51"     //Date of highest committed revision
#define FCRevisionRange "2860:2862"    //Update revision range
#define FCRepositoryURL "https://free-cad.svn.sourceforge.net/svnroot/free-cad/trunk"      //Repository URL of the working copy
#define FCCurrentDateT  "2010/01/16 18:04:24"      //Current system date & time

//Placeholders of the form "$WCxxx?TrueText:FalseText$" are replaced with
//TrueText if the tested condition is true, and FalseText if false.

#define FCScrClean      "Src modified"     //True if local modifications found
#define FCScrMixed      "Src mixed"          //True if mixed update revisions found

