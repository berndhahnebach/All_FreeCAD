                   ...
                   , property->valueText())
{
  ...
  //add children
  Property::List *children = m_property->children();
  if (children) {
    m_children = new Dict();
    Property::ListIterator it(*children);
    PropertyEditorItem *item = 0;
    for (;it.current();++it) {
      item = new PropertyEditorItem(this, it.current(), item);
...
PropertyEditorItem::PropertyEditorItem(QListView *par, const QString &text)
...
/*PropertyEditorItem* PropertyEditorItem::createChildItem(const QString& name)
{
  Property *childProp = m_property->child(name);
  if (!childProp)
    return 0;
  return new PropertyEditorItem(this, childProp);
}*/

/*void
PropertyEditorItem::setValue(QVariant value)
{
...
void
PropertyEditorItem::paintBranches(QPainter *p, const QColorGroup &cg, int w, int y, int h)
{
  p->eraseRect(0,0,w,h);
...
#if 0
QVariant
PropertyEditorItem::getComposedValue()
{
  switch(m_property->type())
  {
  case QVariant::Size:
...
/* let's move this to Property?
void
PropertyEditorItem::updateChildValue()
{
...
void PropertyEditorItem::updateValue(bool alsoParent)
{
  ...
  if (alsoParent && parent())
    static_cast<PropertyEditorItem*>(parent())->updateValue();
}

void
PropertyEditorItem::updateChildrenValue()
{
  //update children value
  PropertyEditorItem *it = static_cast<PropertyEditorItem*>(firstChild());
  while (it) {
    it->updateValue(false);
    it = static_cast<PropertyEditorItem*>(it->nextSibling());
  }
}

/*QString PropertyEditorItem::key( int column, bool ascending ) const
{
  if (!ascending)
...
int PropertyEditorItem::compare( QListViewItem *i, int col, bool ascending ) const
{
  if (!ascending)
    return -QListViewItem::key( col, ascending ).localeAwareCompare( i->key( col, ascending ) );

  return m_order - static_cast<PropertyEditorItem*>(i)->m_order;
