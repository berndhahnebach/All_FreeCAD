...
valueToCursorName(int shape)
{
  switch(shape)
  {
  case Qt::ArrowCursor: return FCTools::i18n("Arrow");
  case Qt::UpArrowCursor: return FCTools::i18n("Up Arrow");
  case Qt::CrossCursor: return FCTools::i18n("Cross");
  case Qt::WaitCursor: return FCTools::i18n("Waiting");
  case Qt::IbeamCursor: return FCTools::i18n("iBeam");
  case Qt::SizeVerCursor: return FCTools::i18n("Size Vertical");
  case Qt::SizeHorCursor: return FCTools::i18n("Size Horizontal");
  case Qt::SizeFDiagCursor: return FCTools::i18n("Size Slash");
  case Qt::SizeBDiagCursor: return FCTools::i18n("Size Backslash");
  case Qt::SizeAllCursor: return FCTools::i18n("Size All");
  case Qt::BlankCursor: return FCTools::i18n("Blank");
  case Qt::SplitVCursor: return FCTools::i18n("Split Vertical");
  case Qt::SplitHCursor: return FCTools::i18n("Split Horizontal");
  case Qt::PointingHandCursor: return FCTools::i18n("Pointing Hand");
  case Qt::ForbiddenCursor: return FCTools::i18n("Forbidden");
  case Qt::WhatsThisCursor: return FCTools::i18n("What's This");
...
Property::Property(const QCString &name, QVariant value, const QString &desc)
{
  m_list = 0;
  m_name = name;
  m_desc = desc;
...
Property::Property(const QCString &name, const QString &value,
...
void Property::init(QVariant value)
{
  m_changed = false;
  m_visible = true;
  m_list = 0;
  ...
  m_parent = 0;
  //  if (!m_desc.isEmpty())
  //    kdDebug() << "Property::Property(): labeled property (" << m_desc << ")" << endl;
  ...
  case QVariant::Size:
    {
      ...
      addChild( new Property("width", QVariant(s.width()), FCTools::i18n("width")) );
      addChild( new Property("height", QVariant(s.height()), FCTools::i18n("height")) );
      break;
    }
  case QVariant::Point:
    {
      QPoint p = value.toPoint();
      addChild( new Property("x", p.x(), FCTools::i18n("x")) );
      addChild( new Property("y", p.y(), FCTools::i18n("y")) );
      break;
    }
  case QVariant::Rect:
    {
      QRect r = value.toRect();
      addChild( new Property("x", r.x(), FCTools::i18n("x")) );
      addChild( new Property("y", r.y(), FCTools::i18n("y")) );
      addChild( new Property("width", r.width(), FCTools::i18n("width")) );
      addChild( new Property("height", r.height(), FCTools::i18n("height")) );
      break;
    }
  case QVariant::SizePolicy:
    {
      ...
      addChild( new Property("horSizeType", spHelper.valueToKey(p.horData()),
                                 spHelper.list, spHelper.list, FCTools::i18n("horSizeType")) );

      addChild( new Property("verSizeType",
                                 spHelper.valueToKey(p.verData()), spHelper.list, spHelper.list, FCTools::i18n("verSizeType")) );

      addChild( new Property("hStretch", (int)p.horStretch(), FCTools::i18n("hStretch") ) );
      addChild( new Property("vStretch", (int)p.verStretch(), FCTools::i18n("vStretch") ) );
      break;
    }
  default:
    {
  ...
  *this = property;
  /*  m_name = property.m_name;
    ...
    if (property.m_children)
      m_children = new Property::Map(*property.m_children);
    else
      m_children = 0;
...
Property::operator=(const Property &property)
{
  ...
  if (property.m_children_list) {
    //copy children
    //m_children_dict = new Property::Dict(property.m_children_dict->size());
    //m_children_dict->setAutoDelete(true);
    //m_children_list = new Property::List();
    Property::ListIterator it(*property.m_children_list);
    for (;it.current();++it) {
      addChild( new Property(*it.current()) );
    }
  } else {
    m_children_dict = 0;
    m_children_list = 0;
...
void Property::setValue(const QVariant &v, bool updateChildren, bool saveOldValue)
{
...
void Property::setValue(const QVariant &v, bool saveOldValue)
{
  setValue(v, true, saveOldValue);
}

void Property::setList(const QStringList &key_list, const QStringList &name_list)
{
...
{
  if (!m_list)
    return Property::format( m_value );
  //special case: return text
  int idx = m_list->keys.findIndex( m_value.toString() );
  if (idx<0) {
    //kdDebug() << "Property::value(): NO SUCH KEY '" << m_value.toString() << "'" << endl;
    return m_value.toString();
  }
  return m_list->names[ idx ];
}

void Property::setChildValue(const QCString& childName, const QVariant &v, bool saveOldValue)
{
  Property * prop = child(childName);
  if (!prop) {
    //kdDebug()<< "Property::setChildValue() NO SUCH CHILD: " << childName << endl;
    return;
  }
  prop->setValue(v, saveOldValue);
}

void Property::updateValueForChild(const QCString& childName,
...
void Property::setChanged(bool set)
{
  if (m_changed==set)
    return;
  m_changed=set;
  if (!m_changed) {
    ...
    if (m_parent && m_parent->m_children_list) {
      Property::ListIterator it(*m_parent->m_children_list);
      for (;it.current();++it) {
...
void Property::addChild(Property *prop)
{
  if (!m_children_dict) {
    m_children_dict = new Property::Dict(53);
    m_children_dict->setAutoDelete(true);
    m_children_list = new Property::List();
  }
...
void Property::debug()
{
  QString dbg = "Property( name='" + QString(m_name) + "' desc='" + m_desc
...
QStringList* Property::keys() const
{
  return m_list ? &m_list->keys : 0;
}

QStringList* Property::names() const
{
  return m_list ? &m_list->names : 0;
}

QString
Property::format(const QVariant &v)
{
  switch(v.type())
  {
  case QVariant::Size:
  ...
  case QVariant::Bool:
    {
      if(v.toBool())
      {
        return FCTools::i18n("True");
      }

      return FCTools::i18n("False");
    }
  case QVariant::Font:
    {
      QFont f = v.toFont();
      int w = f.weight();
      QString wstr;
      if (w<=QFont::Light) wstr=FCTools::i18n("Light")+" ";
      else if (w<=QFont::Normal) wstr="";
      else wstr=FCTools::i18n("Bold")+" ";
      if (f.italic())
        wstr += (FCTools::i18n("Italic")+" ");
