...
MDIView3D::MDIView3D( Gui::Document* pcDocument, QWidget* parent, const char* name, int wflags )
...
*/
const char *MDIView3D::getName(void)
{
  return "View3D";
}
...
*/
void MDIView3D::onWindowActivated ()
{
  //myOperations->onSelectionChanged();
}

void MDIView3D::setCursor(const QCursor& aCursor)
{
...
View3D::View3D(Gui::Document*  pcDocument, QWidget *parent, const char *name, WFlags f)
    :QGLWidget(QGLFormat( QGL::DoubleBuffer| QGL::DepthBuffer| QGL::DirectRendering ),
     parent,name,0,f), _pcDocument(pcDocument)
{
