...
#include <Inventor/Qt/SoQtRenderArea.h>
#include <Inventor/Qt/SoQtGLWidgetP.h> // for eventHandler()
...
public:
  ...
  enum { NONE, OPENGL, INVENTOR, TOOLKIT, DUMPSCENEGRAPH };
...
SoQtRenderAreaP::checkMagicSequences(const char c)
{
  ...
  static const char dumpiv[] = "dumpiv";

  int ml = strlen(glinfoseq);
  if (cl >= ml && this->currentinput.getSubString(cl - ml) == glinfoseq) {
    return SoQtRenderAreaP::OPENGL;
  }
  ml = strlen(ivinfoseq);
  if (cl >= ml && this->currentinput.getSubString(cl - ml) == ivinfoseq) {
    return SoQtRenderAreaP::INVENTOR;
  }
  ml = strlen(soinfoseq);
  if (cl >= ml && this->currentinput.getSubString(cl - ml) == soinfoseq) {
    return SoQtRenderAreaP::TOOLKIT;
  }
  ml = strlen(dumpiv);
  if (cl >= ml && this->currentinput.getSubString(cl - ml) == dumpiv) {
    return SoQtRenderAreaP::DUMPSCENEGRAPH;
  }
...
SoQtRenderAreaP::SoQtRenderAreaP(SoQtRenderArea * pub)
{
  PUBLIC(this) = pub;
...
SoQtRenderArea::redrawOnSelectionChange(SoSelection * selection)
{
  ...
  if (PRIVATE(this)->normalSelection) {
    ...
    PRIVATE(this)->normalSelection->removeChangeCallback(SoQtRenderAreaP::selection_redraw_cb, this);
  }

  PRIVATE(this)->normalSelection = selection;

  if (selection)
    selection->addChangeCallback(SoQtRenderAreaP::selection_redraw_cb, this);
...
SoQtRenderArea::redrawOverlayOnSelectionChange(SoSelection * selection)
{
  ...
  if (PRIVATE(this)->overlaySelection) {
    // FIXME: should we ref() and unref() to make sure we can't get a
    // crash here? 20001002 mortene.
    PRIVATE(this)->overlaySelection->removeChangeCallback(SoQtRenderAreaP::selection_redraw_cb, this);
  }
...
{
#if SOQT_DEBUG && RENDERAREA_DEBUG_REDRAWS // debug
  SoDebugError::postInfo("SoQtRenderArea::render",
...
/*!
  This method redraws the normal scenegraph by calling
