...
SoGuiFullViewerP::prepareMenu(SoQtPopupMenu * menu)
{
  ...
  //// Stereo viewing. ////////////////////////////////////////////////
  SbBool stereo = PUBLIC(this)->isStereoViewing();
  SbBool quadbuffer = PUBLIC(this)->isQuadBufferStereo();
 if (!stereo)
...
SoGuiFullViewerP::menuSelection(int menuitemid)
{
  ...
  case STEREO_OFF_ITEM:
    PUBLIC(this)->setStereoViewing(FALSE);
    PUBLIC(this)->setQuadBufferStereo(FALSE);
    break;
  case STEREO_ANAGLYPH_ITEM:
    PUBLIC(this)->setStereoViewing(TRUE);
    PUBLIC(this)->setQuadBufferStereo(FALSE);
    break;
  case STEREO_QUADBUFFER_ITEM:
    PUBLIC(this)->setStereoViewing(TRUE);
    PUBLIC(this)->setQuadBufferStereo(TRUE);
    if (!PUBLIC(this)->isQuadBufferStereo()) {
      PUBLIC(this)->setStereoViewing(FALSE);
      PUBLIC(this)->prefmenu->setMenuItemMarked(STEREO_OFF_ITEM, TRUE);
    }
    break;
...
// direction vector.
//
// Used from both SoGuiPlaneViewer and
// SoGuiExaminerViewer. Implemented in the SoGuiFullViewer private
// class to collect common code.
void
                      ...
                      const SbVec2f & currpos, const SbVec2f & prevpos)
{
  ...
  if (currpos == prevpos) {
#if SOQT_DEBUG
    SoDebugError::postInfo("SoGuiFullViewerP::pan", "useless invokation");
#endif // SOQT_DEBUG
    return;
  }
