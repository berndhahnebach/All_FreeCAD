...
SoGuiPlaneViewerP::getPointerOrigoAngle(void) const
{
  ...
  float nowradval = (now != SbVec2s(0, 0)) ?
    atan(fabs((float) now[1] / (float) now[0])) : 0.0f;
...
SoGuiPlaneViewerP::getPointerOrigoMotionAngle(void) const
{
  ...
  float thenradval = (then != SbVec2s(0, 0)) ? 
    atan(fabs((float) then[1] / (float) then[0])) : 0.0f;
