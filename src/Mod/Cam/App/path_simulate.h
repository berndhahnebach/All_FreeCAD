...
public:
	...
	/** @brief Computes all parameters of the velocity-function for the tool movement along a straight line
		@param wirelength length of the straight line*/
    bool ParameterCalculation_Line(double wirelenghth);

	/** @brief Computes all parameters of the velocity-function for the tool movement along a curve-segment
		@param wirelength arc-length of the curve-segment*/
	bool ParameterCalculation_Curve(double wirelenghth);
	...
	/** @brief Computes output for the connection-part in all directions for the feature-based-strategy
		...
		@param robo defines outputstyle (false: simulation true: robot)
	...
	/** @brief Main routine for creating the simulation output for a single curve, including the synchronisation of master and slave; CompPath() must have been called before using this function*/
	bool Gen_Path();
    
	/** @brief Creates outputvectors for the simulation outputfor a single curve
	    @param outputstyle false: simulation, true: robot
		@param length defines length of the path
	    @param part tells us that we are generating output for a critical part of the curve or not */ 
	...
	/** @brief Main function of the output-generation for the simulation using the feature-based or spiral-based strategy
	    @param flatAreas index-vector specifying the flat areas 
		@param spiral specifies the strategy (true: spiral-based, false: feature-based)*/
...
private:
	...
	bool m_dir;
	/** @brief vector in wich the lengths of the seperated curve-segments for the master-tool are stored*/
	std::vector<std::vector<double>> m_length_ma;
	/** @brief vector in wich the lengths of the seperated curve-segments for the slave-tool are stored*/
    std::vector<std::vector<double>> m_length_sl;
	/** @brief vector of acceleration-values regarding to the seperated curve-segments for the master-tool*/
    std::vector<std::vector<double>> m_accel_ma;
	/** @brief vector of acceleration-values regarding to the seperated curve-segments for the slave-tool*/
	...
	/** @brief Matrix of three velocity-values regarding to the curves and curve-segments for the slave-tool*/
	...
	double m_amax;
	/** @brief pathtolerance wich is set (in subject to m_vmax and m_amax) before and after a critical region*/
	double m_boundTol;
	/** @brief acceleration-parameter used in GetVelocity() and GetDistance()*/
	double m_a;
     /** @brief three-dimensional vector containing start-, maximum- and end-velocity used in GetVelocity() and GetDistance()*/
	double m_v[3];
