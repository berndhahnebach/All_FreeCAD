...
SpringbackCorrection::SpringbackCorrection(const TopoDS_Shape& aShape, MeshCore::MeshKernel aMesh)
...
SpringbackCorrection::SpringbackCorrection(const TopoDS_Shape& aShape, MeshCore::MeshKernel CadMesh, MeshCore::MeshKernel Mesh)
...
bool SpringbackCorrection::CalcCurv()
{
	...
	for(pntIt.Begin(); pntIt.More(); pntIt.Next())
	{
		aMeshStruct.pnt = *pntIt;             // stores point
		aMeshStruct.index = pntIt->_ulProp;   // stores index
	...
	{  
	  ...
	  MeshingParams._minAngle = 30.0;
	  MeshingParams._minNbPntsPerEdgeLine = 10;
  	  MeshingParams._minNbPntsPerEdgeOther = 10;
	  MeshingParams._minEdgeSplit = 5;
	  
	  BRepMeshAdapt::Mesh(aFace,0.1,MeshingParams);
	  
	  TopLoc_Location aLocation;
	  // takes the triangulation of the face aFace:
	  Handle_Poly_Triangulation aTr = aBRep.Triangulation(aFace,aLocation);
	  if(!aTr.IsNull()) // if this triangulation is not NULL
	  {
		  ...
		  std::vector<Base::Vector3f> TrPoints(aNodes.Length()+1);
		  for( Standard_Integer i = 1; i < aNodes.Length()+1; i++)
		  {
			  aPoints(i) = aNodes(i).Transformed(aLocation);
			  TrPoints[i].x = aPoints.Value(i).X();
			  TrPoints[i].y = aPoints.Value(i).Y();
			  TrPoints[i].z = aPoints.Value(i).Z();
		  }

		  FaceMesh = BuildMesh(aTr, TrPoints);	 		
	  //-----------------------------------------------------------------------------------------------
		  ...
		  // get mesh-boundary
		  GetBoundary(FaceMesh, MeshPnts);

		  // resizing FacePntVector
		  int g = 0;
		  ...
		  FacePntVector.resize(g);  // stores inner-points and sourrounding edges-distances
		  ...
		  for(aExpEdge.Init(aFace,TopAbs_EDGE);aExpEdge.More();aExpEdge.Next())
		  {
			  ...
			  for(pIt = MeshPnts.begin(); pIt != MeshPnts.end(); ++pIt)
			  {
				  ...
				  if(pIt->_ulProp != n)
				  {
					  ...
					  VertexBuild.MakeVertex(V,m_pnt,0.01);
					  pnt2edge.LoadS2(V);
					  pnt2edge.Perform();
					  dist = pnt2edge.Value();
			  ...
			  aBRep.PolygonOnTriangulation(aEdge,polyg,aTr,aloc);
			  siz = polyg->NbNodes();
			  TColStd_Array1OfInteger IndArr(1,siz);
			  IndArr = polyg->Nodes();
			  
			  for(int k=0; k<IndArr.Upper(); ++k)
			  {
		  ...
		  for(int k=0; k<FacePntVector.size(); ++k)
		  {
...
bool SpringbackCorrection::Init()
{
	...
	for(anExplorer.Init(m_Shape,TopAbs_EDGE);anExplorer.More();anExplorer.Next())
	{
		...
		for(;aListIterator.More(); aListIterator.Next())
		{
			...
			best_fit::Tesselate_Face(aFace, FaceMesh, 0.1);
...
bool SpringbackCorrection::GetBoundary(MeshCore::MeshKernel &mesh, MeshCore::MeshPointArray &meshPnts)
{
	// get mesh-boundary
	...
	std::vector <unsigned long>  v_ind;
...
bool SpringbackCorrection::SmoothMesh(MeshCore::MeshKernel &Mesh, double d_max)
{
	...
	for(v_it.Begin(); v_it.More(); v_it.Next())
	{
		...
		N.x = T1[0][0]; N.y = T1[0][1]; N.z = T1[0][2];
...
bool SpringbackCorrection::Perform(int deg_Tol)
{
	...
	CalcCurv();  // berechne Krümmungswerte über Edgekrümmungen

	RefMesh = m_Mesh;  // übegebe CAD-Mesh vor der Verformung
...
std::vector<double> SpringbackCorrection::MeshCurvature(MeshCore::MeshKernel mesh)
{
	// get all points
	...
	m_CurvNeg.clear();
    m_CurvMax.clear();
	m_CurvPos.resize(rMesh.CountPoints());
	m_CurvNeg.resize(rMesh.CountPoints());
	m_CurvMax.resize(rMesh.CountPoints());
	...
	if(c1==0)
		...
		mean_curvMax /= rMesh.CountPoints();

	if(c2==0)
		mean_curvMin = -1e+3;
	else
		mean_curvMin /= rMesh.CountPoints();
...
bool SpringbackCorrection::MirrorMesh(std::vector<double> error)
{
	...
	for(unsigned int j=0; j<SortIndVec.size(); ++j)
	{
		...
		v[0].x = T1[0][0]; v[0].y = T1[0][1]; v[0].z = T1[0][2];
		v[1].x = T1[1][0]; v[1].y = T1[1][1]; v[1].z = T1[1][2];
		v[2].x = T1[2][0]; v[2].y = T1[2][1]; v[2].z = T1[2][2];
		...
		w = 1.0 /(double(NeiLoc.size()) + double(PntNei.size()) - 1.0);
		bvec.Scale(w,w,w);
										  ...
										  std::vector<Base::Vector3f> Normal, bool sign, double minScale)
{
	...
	for(unsigned int k=1; k<Neib.size(); ++k)
	{
		...
		A(0,2) = -Normal[0].x; A(0,0) = Normal[k].x;  A(0,1) = (NeiPntNext.x - NeiPnt.x);
		A(1,2) = -Normal[0].y; A(1,0) = Normal[k].y;  A(1,1) = (NeiPntNext.y - NeiPnt.y);
		A(2,2) = -Normal[0].z; A(2,0) = Normal[k].z;  A(2,1) = (NeiPntNext.z - NeiPnt.z);
	...
	for(unsigned int k=1; k<Neib.size(); ++k)
	{
		...
		A(0,2) = -Normal[0].x; A(0,0) = Normal[k].x; A(0,1) = (NeiPntPrev.x - NeiPnt.x);
		A(1,2) = -Normal[0].y; A(1,0) = Normal[k].y; A(1,1) = (NeiPntPrev.y - NeiPnt.y);
		A(2,2) = -Normal[0].z; A(2,0) = Normal[k].z; A(2,1) = (NeiPntPrev.z - NeiPnt.z);
...
std::vector<Base::Vector3f> SpringbackCorrection::FillConvex(std::vector<Base::Vector3f> Neib, std::vector<Base::Vector3f> Normals, int n)
{
	...
	for(unsigned int i=1; i<Neib.size(); ++i)
	{
		for(int j=0; j<n; ++j)
		{
			//cout << "t, " ;
			tmp = Neib[i] - Neib[0];
			tmp.Scale(double(j)/double(n), double(j)/double(n), double(j)/double(n));
...
bool SpringbackCorrection::InsideCheck(Base::Vector3f pnt, Base::Vector3f normal, std::vector<Base::Vector3f> Neib)
{
	...
	for(unsigned int k=1; k<Neib.size(); ++k)
	{
		...
		A(0,0) = normal.x;  A(0,1) = (NeiPntNext.x - NeiPnt.x); A(0,2) = -Cross.x;
		A(1,0) = normal.y;  A(1,1) = (NeiPntNext.y - NeiPnt.y); A(1,2) = -Cross.y;
		A(2,0) = normal.z;  A(2,1) = (NeiPntNext.z - NeiPnt.z); A(2,2) = -Cross.z; 
