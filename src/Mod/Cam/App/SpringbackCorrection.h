...
/**  @brief a struct for computing the maximum and minimum curvature-radius of a single mesh-point
     @param pnt mesh-point
...
public:
	...
	/** @brief sets parameter-values to initial state, tesselates the input-shape and computes a curvature-radius-value 
	    at each edge contained in the input-shape */ 
    bool Init();
    ...
    bool SetFixEdges();
	/** @brief calculates curvature-values as a distance-weighted convexcombination */
    bool CalcCurv();
    ...
    /** @brief returns index-value wich specifies the boundary-points of the input-mesh
	    @param mesh input-mesh
    ...
    /** @brief smooths input-mesh 
	    @param mesh input-mesh
		@param maxTranslation value wich stands for the maximum deviation from the initial-mesh */
	bool SmoothMesh(MeshCore::MeshKernel &mesh, double maxTranslation);
	/** @brief smooths specified mesh-points of the input-mesh 
	    @param mesh input-mesh
		@param indicies vector of indicies of the mesh-points for smoothing
		@param maxTranslation value wich stands for the maximum deviation from the initial-mesh */
    bool SmoothMesh(MeshCore::MeshKernel &mesh, std::vector<int> indicies, double maxTranslation);
	/** @brief computes current angles of all triangles and stores the angle-degrees in a vector
	    @param mesh input-mesh
		@param deg_Tol limiting forming-angle*/
    //bool GetFaceAng(MeshCore::MeshKernel &mesh, int deg_tol);
	/** @brief computes current angles of all triangles and stores the critical-triangle-indicies in a vector
	    @param mesh input-mesh
		@param deg_Tol limiting forming-angle*/
    std::vector<int> FaceCheck(MeshCore::MeshKernel &mesh, int deg_tol);
	/** @brief computes current angles of all triangles and sets the property-value of the critical triangles to zero
	    @param mesh input-mesh
		@param deg_Tol limiting forming-angle*/
...
private:
    ...
    /** @brief vector containing the minimum curvature-radius-values at all mesh-points*/
    ...
    /** @brief index-vector for the region-growing-algorithm containing the triangles at the current ring-neighbourhood*/
    ...
    /** @brief index-vector for the region-growing-algorithm containing the triangles of all computed region*/
...
/** @brief index wich specifies the current ring-neighbourhood for the region-growing-algorithm*/
    int m_RingCurrent;
private:
    ...
    /** @brief vector containing the distance-values at all mesh points between the initial input-shape (CAD-model) and -mesh*/
