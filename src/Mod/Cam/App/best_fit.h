...
public:
	...
	/*! \brief Determines the center of mass of the input-shape and translates the shape
	...
	*/
	...
	/*! \brief Computes error between mesh and the input-shape through intersection along the normal-directions
	and returns an average-error-value
	*/
	...
	/*! \brief Computes a triangulation on shape
		...
		\param deflection: parameter wich determines the accuracy of the triangulation */
	...
	/*! \brief Returns a normal-vector of Mesh at the knots with uniform weighting
	    \param Mesh Input-Mesh
	*/
    static std::vector<Base::Vector3f> Comp_Normals(MeshCore::MeshKernel &Mesh);
	
	/*! \brief Check and corrects mesh-position by rotating around all coordinate-axes with 180 degree */
    bool Coarse_correction();

	/*! \brief Determines two corresponding point-sets for the ICP-Method using the Nearest-Neighbour-Algorithm */
    double ANN();
...
private:
	/*! \brief Computes the rotation-matrix with reference to the given parameters
	    \param M 4x4-output-matrix
		\param degree rotation-angle in degree
		\param axis rotation-axis (1: x-axis, 
								   2: y-axis, 
								   3: z-axis) */
    inline bool RotMat(Base::Matrix4D &matrix, double degree, int rotaionAxis);
	
	/*! \brief Computes the translation-matrix with reference to the given parameters
		\param M 4x4-output-matrix
		\param trans translation-value
		\param axis translation-axis (1: x-axis, 
		                              2: y-axis, 
									  3: z-axis) */
    inline bool TransMat(Base::Matrix4D &matrix, double translation, int translationAxis);
	
	/*! \brief Tranforms the point-set pnts and the corresponding surface-normals normals with reference to the input-matrix
	...
	/*! \brief Returns the first derivative of the rotation-matrix at the position x
	...
	/*! \brief Returns the second derivative (Hessian matrix) of the rotation-matrix at the position x
