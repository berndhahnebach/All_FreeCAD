...
std::string PovTools::getCamera(const CamDef& Cam)
{
	...
	out 
		...
		<< "// Generated by FreeCAD (http://free-cad.sf.net/)" << endl
		<< "camera {" << endl
		
		// writing Camera positions
		<< "  position <" << Cam.CamPos.X() <<"," << Cam.CamPos.Z() <<"," << Cam.CamPos.Y()<<">" << endl

		// writing Camera Direction vector
		//<< "  look_at <" << Cam.CamDir.X()  <<"," << Cam.CamDir.Z()    <<"," << Cam.CamDir.Y()    <<">" << endl 
	
		// writing lookat
		<< "  look_at <" << Cam.LookAt.X()  <<"," << Cam.LookAt.Z()    <<"," << Cam.LookAt.Y()    <<">"<< endl  

		// writing the Up Vector
		<< "  sky  <" << Cam.Up.X()  <<"," << Cam.Up.Z()    <<"," << Cam.Up.Y()    <<">"<< endl
           ...
           float fMeshDeviation)
{
  ...
  // write the file
  out <<  "// Written by FreeCAD http://free-cad.sf.net/" << endl;
  l = 1;
  for (ex.Init(Shape, TopAbs_FACE); ex.More(); ex.Next(),l++) {
