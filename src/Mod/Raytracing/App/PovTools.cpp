...
void PovTools::writeCamera(const char*   FileName, 
            ...
            const gp_Vec& Up)
{
  std::vector<gp_Vec> vCamPos;
  std::vector<gp_Vec> vCamDir;  
  std::vector<gp_Vec> vLookAt;
  std::vector<gp_Vec> vUp;

  vCamPos.push_back(CamPos);
  vCamDir.push_back(CamDir); 
  vLookAt.push_back(LookAt);
  vUp.push_back(Up);

  writeCameraVec(FileName,vCamPos,vCamDir,vLookAt,vUp);
}
               ...
               const std::vector<gp_Vec>& Up)
{
  ...
  std::vector<gp_Vec>::const_iterator It;


  out << "// declares positon and view directions\n"
      << "// Total number of camera positions\n"
      << "#declare nCamPos = " << CamPos.size() << ";\n\n";

  out << "// Array of positions\n"
      << "#declare  CamPos = array[" << CamPos.size() << "] {\n";
  ...
  out << "};\n"
      ...
      << "#declare  CamDir = array[" << CamDir.size() << "] {\n";

  for(It = CamDir.begin();It != CamDir.end();It++)
    out << "   <" << It->X()  <<"," << It->Z()    <<"," << It->Y()    <<">,\n";
 
  out << "};\n"
      << "// Array of Look At positions\n"
      << "#declare  LookAt = array[" << LookAt.size() << "] {\n";

  for(It = LookAt.begin();It != LookAt.end();It++)
    out << "   <" << It->X()  <<"," << It->Z()    <<"," << It->Y()    <<">,\n";
 
  out << "};\n"
      << "// // Array of up vectors\n"
      << "#declare  Up = array[" << CamDir.size() << "] {\n";
  ...
  out << "};\n"
      << "// // Array of up vectors\n"
      << "#declare  CamZoom = array[" << CamDir.size() << "] {\n";
...
void PovTools::transferToArray(const TopoDS_Face& aFace,gp_Vec** vertices,gp_Vec** vertexnormals, long** cons,int &nbNodesInFace,int &nbTriInFace )
{
 ...
 //     gp_Pnt vertex((*vertices)[i][0], (*vertices)[i][1], (*vertices)[i][2]);
      ...
      if ( temp.DotCross(temp,(*vertexnormals)[i]) < 0 )
        temp = -temp;
