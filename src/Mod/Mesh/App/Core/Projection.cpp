                                         ...
                                         std::vector<SplitEdge>& rSplitEdges )
{
  ...
  std::vector<Vector3D> acPolyLine;
  ...
  if (clDefl.IsDone() == Standard_True)
  {
    ...
    for (Standard_Integer i = 1; i <= nNbPoints; i++)
    {
      ...
      acPolyLine.push_back( Vector3D( gpPt.X(), gpPt.Y(), gpPt.Z() ) );
    }
  }
...
//      continue;
    ...
    Vector3D cPlaneNormal = cEdgeNormal % (cE1 - cE0);
    ...
    if ( Alg.IsDone() )
    {
      ...
      if ( nNbPoints == 1 )
      {
        ...
        if ( l>=0.0f && l<=1.0f )
        {
          Vector3D cSplitPoint = (1-l) * cE0 + l * cE1;
  ...
  // sorted by parameter
  for ( std::map<Quantity_Parameter, SplitEdge>::iterator itS = rParamSplitEdges.begin(); itS != rParamSplitEdges.end(); ++itS )
  {
