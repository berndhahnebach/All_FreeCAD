...
//----------------------------------------------------------------------------
static void ProjectTriangle (const Vector3f& rkD, const Vector3f apkTri[3],
...
//----------------------------------------------------------------------------
static void ProjectBox (const Vector3f& rkD, const Box3f& rkBox, float& rfMin, float& rfMax)
{
    float fDdC = rkD.Dot(rkBox.Center);
    float fR =
        rkBox.Extent[0]*(float)fabs(rkD.Dot(rkBox.Axis[0])) +
        rkBox.Extent[1]*(float)fabs(rkD.Dot(rkBox.Axis[1])) +
        rkBox.Extent[2]*(float)fabs(rkD.Dot(rkBox.Axis[2]));
    rfMin = fDdC - fR;
    rfMax = fDdC + fR;
}
//----------------------------------------------------------------------------
bool TestIntersection (const Vector3f apkTri[3], const Box3f& rkBox)
{
...
//----------------------------------------------------------------------------
static bool NoIntersect (float fTMax, float fSpeed, float fMin0, float fMax0,
...
//----------------------------------------------------------------------------
bool /*Mgc::*/TestIntersection (const Vector3f apkTri[3], const Vector3f& rkTriVel,
    const Box3f& rkBox, const Vector3f& rkBoxVel, float fTMax, float& rfTFirst, float& rfTLast)
{
    float fMin0, fMax0, fMin1, fMax1, fSpeed;
    Vector3f kD, akE[3];

    // process as if triangle is stationary, box is moving
    Vector3f kW = rkBoxVel - rkTriVel;
    rfTFirst = 0.0f;
    rfTLast = FLOAT_MAX/*Math::MAX_REAL*/;
...
/// @todo Implement
bool MeshFacetFunc::IntersectWithBoundingBox (const MeshGeomFacet &rclFacet, const BoundBox3D &rclBB)
{
  const Vector3D& rclP0 = rclFacet._aclPoints[0];
  const Vector3D& rclP1 = rclFacet._aclPoints[1];
  const Vector3D& rclP2 = rclFacet._aclPoints[2];

  Vector3<float> akTri[3] = {
                          Vector3<float>(rclP0.x, rclP0.y, rclP0.z), 
                          Vector3<float>(rclP1.x, rclP1.y, rclP1.z), 
                          Vector3<float>(rclP2.x, rclP2.y, rclP2.z)
                        };
...
/// @todo
//  IntrSegment3Box3<float> intrsectbox(akSeg, kBox, true);
//  return intrsectbox.Test();
  return TestIntersection(akTri, kBox);
}
...
bool MeshFacetFunc::IntersectPlaneWithLine (const MeshGeomFacet &rclFacet, const Vector3D &rclPt, const Vector3D &rclDir, Vector3D &rclRes)
{
  ...
  if ( rclDir * rclFacet.GetNormal() < 1e-3f )
...
/// @todo test
float MeshFacetFunc::DistanceToPoint (const MeshGeomFacet &rclFacet, const Vector3D &rclPt, float &s, float &t)
{
  ...
  akDistPtTria.Get(s, akF0, akF1);
  akDistPtTria.Get(t, akF0, akF2);

  return akDistPtTria.Get();
}

/// @todo test
float MeshFacetFunc::MinLineLine (const Vector3D &r0B, const Vector3D &r0M,
                                  const Vector3D &r1B, const Vector3D &r1M,
                                  float &s, float &t)
{
  Vector3<float> A(r0B.x, r0B.y, r0B.z);
  Vector3<float> B(r0M.x, r0M.y, r0M.z);
  Vector3<float> C(r1B.x, r1B.y, r1B.z);
  Vector3<float> D(r1M.x, r1M.y, r1M.z);

  // Build up the line segments
  Vector3<float> p0(0.5f*(A+B));
  Vector3<float> p1(0.5f*(C+D));

  // edge lengths
  float len0 = (r0B-r0M).Length();
  float len1 = (r1B-r1M).Length();

  Vector3<float> d0(r0M.x - r0B.x, r0M.y - r0B.y, r0M.z - r0B.z);
  d0.Normalize();
  Vector3<float> d1(r1M.x - r1B.x, r1M.y - r1B.y, r1M.z - r1B.z);
  d1.Normalize();

  Segment3<float> akSeg0(p0, d0, len0/2.0f  );
  Segment3<float> akSeg1(p1, d1, len1/2.0f);

  DistSegment3Segment3<float> akDistSegSeg(akSeg0, akSeg1);
  akDistSegSeg.Get(s, A, B);
  akDistSegSeg.Get(t, C, D);

  return akDistSegSeg.Get();
}
...
bool MeshEdgeFunc::ContainedByOrIntersectWithBoundingBox (const MeshGeomEdge &rclEdge, 
                                                          const BoundBox3D &rclBB )
{
  ...
  if (MeshEdgeFunc::IntersectWithBoundingBox(rclEdge, rclBB))
    return true;
...
/// @todo testen
bool MeshEdgeFunc::IntersectWithBoundingBox (const MeshGeomEdge &rclEdge, const BoundBox3D &rclBB)
{
