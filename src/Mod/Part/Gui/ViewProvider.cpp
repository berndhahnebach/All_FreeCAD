...
//	BRepMesh_Discret MESH(1.0,myShape,20.0);
	BRepMesh_IncrementalMesh MESH(myShape,0.5);
...
/*
  BRepAdaptor_Surface clSurface(aFace);
  GeomAbs_SurfaceType eType = clSurface.GetType();

  Standard_Real fU=0, fV=0; 
  fU = (float)(clSurface.FirstUParameter());
  fV = (float)(clSurface.LastUParameter());
...
//  GeomAPI_ProjectPointOnSurf ProPntSrf(gp_Pnt(clSTLPoint.x, clSTLPoint.y, clSTLPoint.z), Surface);
//  ProPntSrf.Parameters(1, fU, fV);
  GeomLProp_SLProps clPropOfFace(Surface, (Umax+Umin)/2.0, (Vmin+Vmax)/2.0, 2, gp::Resolution());

  gp_Dir clFaceNormal;
  try{
    clFaceNormal = clPropOfFace.Normal();
  }
