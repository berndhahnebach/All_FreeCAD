...
/* OCC: we include Triangle.hxx first since it has to define REAL independently */
/* from this C file... This ensures that definition in Triangle.hxx is the same */
/* as when triangle.c is compiled */
...
#endif /* LINUX */
#ifdef TRILIBRARY
/*#include "triangle.h"   -- not needed because of included Triangle.hxx above */
#endif /* TRILIBRARY */
...
struct behavior {
	...
	REAL maxarea;
	/* Modified by mpv Dec 25 2006 OCC14376: add this flag to increase precision for some cases */
	int increase_precision;
...

{
	...
	/* Modified by mpv Dec 25 2006 OCC14376: Begin */
	if (!b->increase_precision) 
	{
	...
	if (!b->increase_precision) 
	{ /* do not do not precise checking */
	...
	/* Modified by mpv Dec 25 2006 OCC14376: End */
...

{
	...
	triangle ptr;                         /* Temporary variable used by sym(). */
	subseg sptr;                      /* Temporary variable used by tspivot(). */
	triangle *looptri; /* Open CASCADE Support debug */
	...
	looptri = searchtri->tri; /* Open CASCADE Support debug */
	while (1) {
		if (b->verbose > 2) {
		...
		/* Open CASCADE Support debug : Check for endless loop */
		if (searchtri->tri == looptri)
		{
			/* Try most safe return */
			return ONVERTEX;
		}
...

{
	...
	if (ahead < 0.0) {
		...
		/* Open CASCADE Support debug */
		/*symself(*searchtri);*/
		sym(*searchtri,sampletri);
		if (sampletri.tri != m->dummytri)
			otricopy(sampletri,*searchtri);
	} else if (ahead == 0.0) {
...

{
	...
	/*m->eextras = 0; Open CASCADE Support debug*/
...

{
	...
	/* jfa 9.04.2004 As variable initialisation is commented in delaunay() method */
	/* with <Open CASCADE Support debug> comment */
	m.eextras = 0;
...
/*****************************************************************************/
/*                                                                           */
/* Open CASCADE extensions of triangle.c                                     */
/*                                                                           */
/* September 10, 2003                                                        */
/*                                                                           */
/*****************************************************************************/
...
/*****************************************************************************/
/*                                                                           */
/* Static variables used by Open CASCADE extensions                          */
/*                                                                           */
/*****************************************************************************/

struct behavior occbehavior;
struct mesh occmesh;

struct behavior *b = &occbehavior;
struct mesh *m = &occmesh;

REAL *holearray = (REAL *) NULL;
REAL *regionarray = (REAL *) NULL;

/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_InitBehavior                                                     */
/*                                                                           */
/*****************************************************************************/

void triangleAdapt_InitBehavior(double minangle, double maxarea)
{
	/*
	b->poly = b->refine = b->quality = 0;
	b->vararea = b->fixedarea = b->usertest = 0;
	...
	*/
	...
	b->vararea = 0;
	b->fixedarea = 0;
	b->usertest = 0; /* since Version 1.4 */

	b->regionattrib = 0;
	b->convex = 0;
	b->weighted = 0; /* since Version 1.4 */
	b->jettison = 0; /* since Version 1.4 */
	...
	b->noholes = 0;
	b->noexact = 0;
	b->conformdel = 0; /* since Version 1.4 */
	...
	b->minangle = 0.0;
	b->goodangle = 1.0; /* squared cos(b->minangle * PI / 180.0); */
	b->maxarea = -1;

	/* Modified by mpv Dec 25 2006 OCC14376: normal precision by default */
	b->increase_precision = 0;
	...
	if ( minangle > 0.0 )
	{
		...
		b->goodangle = cos(b->minangle * PI / 180.0);
		b->goodangle *= b->goodangle;
		b->conformdel = 1; /* since Version 1.4 */
		b->nobisect = 1;
	}
...
/*
	if ( maxedgelen > 0 )
	{
		b->quality = 1;
		b->usertest = 1;
		b->sqmaxedgelength = maxedgelen*maxedgelen;
	}
*/	//b->vararea = 1;
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_InitMesh                                                         */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
void triangleAdapt_InitMesh(int nbnodes)
#else /* not ANSI_DECLARATORS */
void triangleAdapt_InitMesh(nbnodes)
int nbnodes;
#endif /* not ANSI_DECLARATORS */
{
	triangleinit(m);
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_AddPointIni                                                      */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
void triangleAdapt_AddPointIni(REAL x, REAL y, int index)
#else /* not ANSI_DECLARATORS */
void triangleAdapt_AddPointIni(x, y, index)
REAL x;
REAL y;
int index;
#endif /* not ANSI_DECLARATORS */
{
	...
	pointloop = (vertex) poolalloc(&m->vertices);
	pointloop[0] = x;
	pointloop[1] = y;
	pointloop[2] = index;
	setvertexmark(pointloop, 0);
	setvertextype(pointloop, INPUTVERTEX);

	/* Determine the smallest and largest x and y coordinates. */
	if (m->vertices.items == 1)
	{
		m->xmin = m->xmax = x;
		m->ymin = m->ymax = y;
	}
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_AddPoint                                                         */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
int triangleAdapt_AddPoint(REAL x, REAL y, int index)
#else /* not ANSI_DECLARATORS */
int triangleAdapt_AddPoint(x, y, index)
REAL x;
REAL y;
int index;
#endif /* not ANSI_DECLARATORS */
{
	vertex pointloop;
	struct otri starttri;
	enum insertvertexresult res;
	int success = 1;
	/*triangle ptr;*/ /* Temporary variable used by sym...() macros */

	/* Read the node */
	pointloop = (vertex) poolalloc(&m->vertices);
	pointloop[0] = x;
	pointloop[1] = y;
	pointloop[2] = (REAL)index;
	setvertexmark(pointloop, 0);
	setvertextype(pointloop, INPUTVERTEX);
	...
	m->invertices++;
	res = insertvertex(m,b,pointloop,&starttri,(struct osub *)NULL,0,0);

	if (res == VIOLATINGVERTEX || res == DUPLICATEVERTEX)
	{
		vertexdealloc(m,pointloop);
		m->invertices--;
		success = 0;
	}

	/* Ensure that no point can be mistaken for a triangular bounding */
	/* box point in insertvertex().                                   */
	m->infvertex1 = (vertex) NULL;
	m->infvertex2 = (vertex) NULL;
	m->infvertex3 = (vertex) NULL;
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_AddEdge                                                          */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
void triangleAdapt_AddEdge(const int *nodes, int length, int orient, int first)
#else /* not ANSI_DECLARATORS */
void triangleAdapt_AddEdge(nodes, length, orient, first)
const int *nodes;
int length;
int orient;
int first;
#endif /* not ANSI_DECLARATORS */
{
	...
	for (i = 1; i < length; i++)
	{
		...
		if (orient == 1) /* TopAbs_REVERSED */
		{
			endpoint1 = getvertex(m, b, end2);
			endpoint2 = getvertex(m, b, end1);
		}
		else
		{
			endpoint1 = getvertex(m, b, end1);
		...
		m->insegments++;
		insertsegment(m, b, endpoint1, endpoint2, 1);
		end1 = end2;
	}
}
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_InsertHoles                                                      */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
void triangleAdapt_InsertHoles(REAL *arrholes, int length)
#else /* not ANSI_DECLARATORS */
void triangleAdapt_InsertHoles(arrholes, length)
REAL *arrholes;
int length;
#endif /* not ANSI_DECLARATORS */
{
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_GetMeshSize                                                      */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
void triangleAdapt_GetMeshSize(int* nbnodes, int* nbtriangles)
#else /* not ANSI_DECLARATORS */
void triangleAdapt_GetMeshSize(nbnodes, nbtriangles)
int *nbnodes;
int *nbtriangles;
#endif /* not ANSI_DECLARATORS */
{
	/* Do not count undead vertices */
	*nbnodes = m->vertices.items - m->undeads;
	*nbtriangles = m->triangles.items;
}
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_NextPoint                                                        */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
int triangleAdapt_NextPoint(int vmark, REAL *x, REAL *y, int *index,int *type)
#else /* not ANSI_DECLARATORS */
int triangleAdapt_NextPoint(vmark, x, y, index)
int vmark;
REAL *x;
REAL *y;
int *index;
#endif /* not ANSI_DECLARATORS */
{
	...
	pointloop = vertextraverse(m);

	while (pointloop != (vertex) NULL)
	{
		/* It is necessary to ignore undead vertices */
		int theType = vertextype(pointloop);
		*type = theType;
		if ( theType != UNDEADVERTEX)
		{
...
void triangleAdapt_InitTriangles()
{
	traversalinit(&m->triangles);
}
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_NextTriangle                                                     */
/*                                                                           */
/*****************************************************************************/

#ifdef ANSI_DECLARATORS
int triangleAdapt_NextTriangle(int *n1, int *n2, int *n3, int *m1, int *m2, int *m3)
#else /* not ANSI_DECLARATORS */
int triangleAdapt_NextTriangle(n1, n2, n3, m1, m2, m3)
int *n1;
int *n2;
int *n3;
int *m1;
...
#endif /* not ANSI_DECLARATORS */
{
	...
	if (triangleloop.tri == (triangle *) NULL) return 0;
	...
	*n3 = vertexmark(p3);
	...
	if (b->usesegments)
	{
		...
		sdecode(triangleloop.tri[6], ssloop);
		*m1 = mark(ssloop);
		sdecode(triangleloop.tri[7], ssloop);
		*m2 = mark(ssloop);
		sdecode(triangleloop.tri[8], ssloop);
		*m3 = mark(ssloop);
	}
	else
	{
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_Triangulate                                                      */
/*                                                                           */
/*****************************************************************************/

int triangleAdapt_Triangulate()
{
	m->xminextreme = 10 * m->xmin - 9 * m->xmax;

	m->checksegments = 0; /* There are no segments in the triangulation yet. */

	m->hullsize = delaunay(m,b); /* Triangulate the nodes. */
	...
	return (m->triangles.items > 0); /* There are some triangles */
}
...
void triangleAdapt_FinishTriangulation()
{
	/* Carve out holes and concavities. */
	carveholes(m, b, holearray, m->holes, regionarray, m->regions);
...
void triangleAdapt_FreeTriangulation()
{
	if (m->holes > 0)
	{
		free(holearray);
...
/*****************************************************************************/
/*                                                                           */
/* triangleAdapt_IncreasePrecision                                                */
/*                                                                           */
/*****************************************************************************/

/* Modified by mpv Dec 25 2006 OCC14376: Begin */
void triangleAdapt_IncreasePrecision()
{
	b->increase_precision = 1;
}
/* Modified by mpv Dec 25 2006 OCC14376: End */
