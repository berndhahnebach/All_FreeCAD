#include <BRepMeshAdapt_ShapeTool.ixx>
...
BRepMeshAdapt_ShapeTool::BRepMeshAdapt_ShapeTool() {}

Standard_Boolean BRepMeshAdapt_ShapeTool::MoreInternalVertex() 
{
...
TopoDS_Vertex BRepMeshAdapt_ShapeTool::FirstVertex(const TopoDS_Edge& E)
{
...
TopoDS_Vertex BRepMeshAdapt_ShapeTool::LastVertex(const TopoDS_Edge& E)
{
  TopExp_Explorer Ex(E,TopAbs_VERTEX);
  while (Ex.More()) {
...
void BRepMeshAdapt_ShapeTool::Vertices(const TopoDS_Edge& E,
...
Bnd_Box BRepMeshAdapt_ShapeTool::Bound(const TopoDS_Face& F)
{
  Bnd_Box Bf;
  BRepBndLib::Add(F, Bf);
  return Bf;
...
Bnd_Box BRepMeshAdapt_ShapeTool::Bound(const TopoDS_Edge& E)
{
  Bnd_Box Be;
  BRepBndLib::Add(E, Be);
  return Be;
...
void  BRepMeshAdapt_ShapeTool::Parameters(const TopoDS_Edge& E,
				     const TopoDS_Face& F,
...
void  BRepMeshAdapt_ShapeTool::Locate(const BRepAdaptor_Curve& C,
				 const Standard_Real W,
				 ...
				 gp_Pnt2d& UV)
{
  ...
  if (pcos.IsDone()) {
    ...
    if (debug!=0) {
      if (pcos.Value()>(2.* C.Tolerance()))  {
	cout << " ShapeTool :LocateExtPCOnS Done but (Distance "<< 
	  pcos.Value() << ")(Tolerance "<<C.Tolerance()<<")" << endl;
	cout << "                W donné : "<< W<< " W calculé : "<<  
	  wFound << endl;
      }
      else if (debug>1) {
...
void BRepMeshAdapt_ShapeTool::AddInFace(const TopoDS_Face&          F,
